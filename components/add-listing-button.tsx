...
function TextInput({ value, onChange, onBlur, placeholder, type = "text" }: { value: string; onChange: (v: string) => void; onBlur?: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <input type={type} value={value} onChange={(e) => onChange(e.target.value)} onBlur={(e) => onBlur?.(e.target.value)}
      ...
      if (res.id && (photoFiles.length > 0 || agreementFiles.length > 0)) {
        ...
        try {
          ...
          if (agreementFiles.length > 0) {
            ...
            if (aUrls.length > 0) await saveOwnerLeadAgreementUrl(res.id, serializeAgreementUrls(aUrls));
          }
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
          ...
          <div className="relative z-10 w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl p-6 overflow-y-auto max-h-[90dvh]"
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <FieldLabel required>Expected rent (RM/mo)</FieldLabel>
                  ...
                  {/* Parking */}
                  <div className="grid grid-cols-2 gap-3">
                    <div><FieldLabel>Parking</FieldLabel><TextInput value={form.parking} onChange={(v) => setForm((f) => ({ ...f, parking: v }))} placeholder="e.g. A142" /></div>
                  </div>
              ...
              {/* Agreement — up to 3 files, any type */}
              <div>
                <SectionLabel>Agreement <span className="normal-case font-normal">(optional, max 3 files)</span></SectionLabel>
