...
export function CompetitorTimeline({ leads, onMonthClick, selectedMonth }: Props) {
  ...
  const buckets = useMemo(() => {
    ...
    months.forEach((m) => map.set(m.key, { count: 0, amount: 0 }));
    leads.forEach((l) => {
      if (!l.competitor_contract_end) return;
      const key = l.competitor_contract_end.slice(0, 7);
      if (map.has(key)) {
        const cur = map.get(key)!;
        map.set(key, { count: cur.count + 1, amount: cur.amount + (l.expected_rent ?? 0) });
      }
    });
    return map;
  }, [leads, months]);
  ...
  const totalRm = Array.from(buckets.values()).reduce((sum, b) => sum + b.amount, 0);
        ...
        <div style={{ display: "grid", gridTemplateColumns: `repeat(${windowMonths}, minmax(36px, 1fr))`, gap: "0.5rem", minWidth: `${Math.max(windowMonths * 46, 300)}px` }}>
          {months.map((m) => {
            ...
            return (
              <div
                key={m.key}
                title={`${m.label} ${m.year}: ${b.count} unit${b.count === 1 ? "" : "s"} · RM ${b.amount.toLocaleString()} potential`}
              ...
              >
                ...
                <div className="text-center" style={{ height: 56 }}>
                  ...
                  <p className="text-[10px] mt-0.5 tabular-nums leading-none" style={{ color: "var(--kk-ink-mute)", visibility: b.amount > 0 ? "visible" : "hidden" }}>
                    RM {b.amount > 0 ? fmtRm(b.amount) : "—"}
                  </p>
                </div>
              </div>
            );
