        ...
        <>
          ...
          <div className="lg:hidden flex flex-col gap-3">
            ...
            {visibleManaged.map((t) => {
              ...
              return (
                <Link key={t.tenancy_id} href={`/existing-listing?open=${t.tenancy_id}`} className="kk-card kk-card-hover p-3 block">
                  <div className="flex gap-2.5">
                    <div className="w-[52px] h-[52px] rounded-xl overflow-hidden shrink-0 flex items-center justify-center"
                      style={{ background: "var(--kk-surface-2)", border: "1px solid var(--kk-line)" }}>
                      <Building2 className="w-4 h-4" style={{ color: "var(--kk-ink-faint)" }} />
                ...
                {visibleManaged.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/existing-listing?open=${t.tenancy_id}`} onClick={(e) => e.stopPropagation()}>
                          <ArrowRight className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" style={{ color: "var(--kk-ink-mute)" }} />
                        </Link>
                      </td>
                    </tr>
      ...
      <div className="kk-card w-full max-w-md flex flex-col" style={{ maxHeight: "90vh", overflow: "hidden" }}>
        ...
        <div className="p-5 border-t flex flex-col gap-2" style={{ borderColor: "var(--kk-line)" }}>
          ...
          {lead.stage === "matched" && tenantInfo ? (
            <Link href={`/existing-listing?open=${tenantInfo.tenancy_id}`} onClick={onClose}
              className="flex items-center justify-center gap-2 w-full py-2.5 rounded-2xl text-[13px] font-semibold"
