...
import { restoreTenancyAction, hardDeleteTenancyAction } from "@/lib/actions";
...
export function DeletedTenanciesPanel({ tenancies }: { tenancies: Tenancy[] }) {
  ...
  async function handleHardDelete(id: string) {
    ...
    try {
      await hardDeleteTenancyAction(id);
      setConfirmId(null);
      router.refresh();
    } finally {
    ...
    <div className="mt-6 mx-auto max-w-[1440px]">
      ...
      {open && (
        <div className="kk-section mt-2 overflow-hidden p-0">
          {tenancies.map((t, i) => {
            const isLast = i === tenancies.length - 1;
            const isConfirming = confirmId === t.id;
            const propertyLabel = t.property_name ?? (t.property?.address ?? "—");
            return (
              <div
                key={t.id}
              ...
              >
                ...
                {!isConfirming ? (
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    >
                      {restoringId === t.id ? <Loader2 className="w-3 h-3 animate-spin" /> : <RotateCcw className="w-3 h-3" />}
                      Restore
                    </button>
                    <button
                      type="button"
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    >
                      {hardDeletingId === t.id && <Loader2 className="w-3 h-3 animate-spin" />}
                      Delete forever
                    </button>
                  </div>
                )}
