...
function SignUpForm() {
  ...
  const [form, setForm] = useState({ email: "", passcode: "", referralCode: refSlug })
  ...
  function set(field: "email" | "passcode" | "referralCode") {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      ph?.capture("user_signed_up", { email: form.email, referral_slug: form.referralCode.trim() || null })
      setLoading(false)
      setEmailSent(true)
    } catch {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
          ...
          <div className="flex flex-col gap-1.5">
            <Label style={{ fontSize: "var(--kk-sm)", color: "var(--kk-ink)" }}>
              Referral code <span style={{ color: "var(--kk-ink-faint)", fontWeight: 400 }}>(optional)</span>
            </Label>
            <Input
              type="text"
              ...
              style={{ ...sharedInputStyle, textTransform: "uppercase", letterSpacing: "0.06em" }}
            />
          </div>
